...
void setBlackOrangeTheme(QApplication &app) {
  ...
  darkPalette.setColor(QPalette::Inactive, QPalette::Highlight,
                       QColor(180, 100, 0));
  darkPalette.setColor(QPalette::Inactive, QPalette::HighlightedText,
                       veryLightGray);
...
int main(int argc, char *argv[]) {
  ...
  int sfBoldId =
      QFontDatabase::addApplicationFont(":/fonts/SF-Pro-Display-Bold.otf");
  int sfMediumId =
      QFontDatabase::addApplicationFont(":/fonts/SF-Pro-Display-Medium.otf");

  if (sfRegularId != -1) {
    // SF Pro fonts loaded successfully
    QStringList fontFamilies =
        QFontDatabase::applicationFontFamilies(sfRegularId);
