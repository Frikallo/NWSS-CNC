...
void DesignerView::setSvgFile(const QString &filePath) {
  ...
  if (!contentBounds.isEmpty) {
    ...
    if (originalFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
      ...
      QString modifiedSvgContent =
          createCroppedSvg(originalSvgContent, contentBounds);
  ...
  qDebug() << "SVG renderer created successfully. Default size:"
           << m_renderer->defaultSize();
  ...
  // Set SVG bounds to the renderer's default size (which is now the content
  // size if we cropped)
  m_svgBounds = QRectF(QPointF(0, 0), m_renderer->defaultSize());
  m_originalSvgBounds =
      m_svgBounds;  // Store original bounds for scaling reference
  ...
  if (m_baseSize.width() > 1000 || m_baseSize.height() > 1000) {
    ...
    m_baseSize =
        QSize(m_baseSize.width() * factor, m_baseSize.height() * factor);
  }
  ...
  // If we have content bounds (margin removal), auto-scale to fit material with
  // padding
  if (m_hasContentBounds) {
    // Use the renderer's default size (in mm) for scaling calculations instead
    // of raw content bounds
    ...
    qDebug() << "Auto-scaling check: rendered SVG=" << renderedWidth << "x"
             << renderedHeight << "mm, material=" << m_materialWidth << "x"
             << m_materialHeight << "mm";
    ...
    qDebug() << "Scale factors: scaleX=" << scaleX << "scaleY=" << scaleY
             << "autoScale=" << autoScale;
    ...
    if (autoScale < 1.0) {
      ...
      m_currentDesignBounds =
          QRectF(centerX - newWidth / 2.0, centerY - newHeight / 2.0, newWidth,
                 newHeight);

      qDebug() << "Auto-scaled content from" << contentBounds.width << "x"
               << contentBounds.height << "to" << newWidth << "x" << newHeight
               << "to fit material";
    }
  }
...
QString DesignerView::createCroppedSvg(
    const QString &originalSvg,
    const nwss::cnc::SVGContentBounds &contentBounds) {
  ...
  QString croppedSvg =
      QString(
          ...
          "<svg width=\"%1mm\" height=\"%2mm\" viewBox=\"0 0 %3 %4\" "
          ...
          .arg(QString::number(contentBounds.width, 'f', 3))
          .arg(QString::number(contentBounds.height, 'f', 3))
          .arg(QString::number(-contentBounds.minX, 'f',
                               3))  // Negative to translate to origin
          .arg(QString::number(-contentBounds.minY, 'f',
                               3))  // Negative to translate to origin
          .arg(extractSvgContent(originalSvg));

  qDebug() << "Creating cropped SVG with transform translate("
...
QString DesignerView::extractSvgContent(const QString &originalSvg) {
  // Extract content between <svg> and </svg> tags, excluding the svg element
  // itself
  QRegularExpression svgContentRegex(
      "<svg[^>]*>(.*)</svg>", QRegularExpression::DotMatchesEverythingOption);
  ...
  switch (m_currentTool) {
    case Select:
      ...
      if (m_designBoundingBox && m_designBoundingBox->isVisible()) {
        ...
        if (m_designBoundingBox->contains(
                m_designBoundingBox->mapFromScene(scenePos))) {
    ...
    case Select:
      if (m_isResizing && (event->buttons() & Qt::LeftButton)) {
        ...
        if (m_designBoundingBox && m_designBoundingBox->isVisible()) {
          ...
          if (handleIndex >= 0) {
            setCursor(Qt::SizeFDiagCursor);
          } else if (m_designBoundingBox->contains(
                         m_designBoundingBox->mapFromScene(scenePos))) {
            setCursor(Qt::SizeAllCursor);
          } else {
            // Show hand cursor over empty space to indicate panning is
            // available
            setCursor(Qt::OpenHandCursor);
          }
        } else {
...
void DesignerView::drawMaterialBoundary(QPainter *painter) {
  ...
  double baseFontSize =
      qMin(minDimension * 0.1, 12.0);  // 10% of smallest dimension, max 12pt
  ...
  QPair<QRectF, QRectF> labelPositions =
      calculateLabelPositions(label, bedLabel, font, bedFont);
...
void DesignerView::drawBedBoundary(QPainter *painter) {
  ...
  double baseFontSize =
      qMin(minDimension * 0.08, 10.0);  // 8% of smallest dimension, max 10pt
  double fontSize = getZoomAdjustedSize(baseFontSize);
  font.setPointSize(qRound(fontSize));
  painter->setFont(font);
  ...
  QPair<QRectF, QRectF> labelPositions =
      calculateLabelPositions(materialLabel, label, materialFont, font);
...
void DesignerView::drawMeasureLine(QPainter *painter) {
  ...
  textRect.adjust(-padding, -getZoomAdjustedSize(2.0), padding,
                  getZoomAdjustedSize(2.0));

  // Draw text background
  painter->setBrush(QColor(40, 40, 40, 220));
  painter->setPen(Qt::NoPen);
  painter->drawRoundedRect(textRect, getZoomAdjustedSize(2.0),
                           getZoomAdjustedSize(2.0));
...
void DesignerView::updateBoundaries() {
  ...
  // Add padding
  double padding = 50.0;
  QRectF sceneRect(-padding, -padding, maxWidth + 2 * padding,
                   maxHeight + 2 * padding);
...
void DesignerView::exportAsSvg(const QString &filePath) {
  ...
  m_scene->render(&painter, QRectF(),
                  QRectF(0, 0, m_materialWidth, m_materialHeight));
...
void DesignerView::createDesignBoundingBox() {
  ...
  m_designBoundingBox->setPen(
      QPen(QColor(255, 140, 0), getZoomAdjustedSize(2.0), Qt::DashLine));
  ...
  m_widthLabel->setFont(
      QFont("Arial", qRound(getZoomAdjustedSize(10.0)), QFont::Bold));
...
void DesignerView::createResizeHandles() {
  ...
  // Create 8 resize handles (corners and midpoints) with smaller, thinner
  // design
  for (int i = 0; i < 8; ++i) {
    QGraphicsRectItem *handle = new QGraphicsRectItem();
    double handleSize =
        getZoomAdjustedSize(HANDLE_SIZE);  // Make handles smaller
    handle->setRect(-handleSize / 2, -handleSize / 2, handleSize, handleSize);
    handle->setPen(
        QPen(QColor(255, 140, 0), getZoomAdjustedSize(2.0)));  // Thinner pen
...
void DesignerView::updateResizeHandles() {
  ...
  for (int i = 0; i < 8 && i < m_resizeHandles.size(); ++i) {
    ...
    // Update handle size and pen width based on zoom - use smaller, thinner
    // design
    double handleSize =
        getZoomAdjustedSize(HANDLE_SIZE);  // Make handles smaller
    handle->setRect(-handleSize / 2, -handleSize / 2, handleSize, handleSize);
...
void DesignerView::updateSizeLabels() {
  ...
  int fontSize =
      qMax(1, qRound(getZoomAdjustedSize(10.0)));  // Ensure minimum size of 1
  ...
  // Rotate height label 90 degrees and position it to the left of the bounding
  // box
  m_heightLabel->setRotation(-90);

  // Calculate the proper position for the rotated height label
  // The rotated text's width becomes the height in the original coordinate
  // system
  double rotatedTextWidth = m_heightLabel->boundingRect()
                                .height();  // Height becomes width when rotated
  double minSpacing =
      getZoomAdjustedSize(15.0);  // Minimum spacing to ensure it's outside
  double requiredSpacing =
      qMax(minSpacing, rotatedTextWidth + getZoomAdjustedSize(5.0));
...
void DesignerView::performResize(const QPointF &currentPos) {
  ...
  if (newRect.width() < MIN_DESIGN_SIZE) {
    if (m_activeResizeHandle == 0 || m_activeResizeHandle == 6 ||
        m_activeResizeHandle == 7) {
  ...
  if (newRect.height() < MIN_DESIGN_SIZE) {
    if (m_activeResizeHandle == 0 || m_activeResizeHandle == 1 ||
        m_activeResizeHandle == 2) {
...
QList<QGraphicsItem *> DesignerView::getSvgItems() const { return m_svgItems; }
...
void DesignerView::updateSvgItemsTransform() {
  ...
  QPointF targetPos = m_currentDesignBounds.center() -
                      QPointF(scaledWidth / 2, scaledHeight / 2);
...
SVGDesigner::SVGDesigner(QWidget *parent) : QWidget(parent) {
  // Create main layout
  ...
  connect(m_designerView, &DesignerView::zoomChanged, this,
          &SVGDesigner::onSvgViewZoomChanged);

  // Connect measure updated signal
  connect(m_designerView, &DesignerView::measureUpdated, this,
          &SVGDesigner::onMeasureUpdated);

  // Connect design bounds changed signal
  connect(m_designerView, &DesignerView::designBoundsChanged, this,
          &SVGDesigner::onDesignBoundsChanged);
...
void SVGDesigner::setupDesignerToolbar() {
  ...
  connect(m_openButton, &QPushButton::clicked, this,
          &SVGDesigner::onOpenSvgClicked);
  connect(m_zoomSlider, &QSlider::valueChanged, this,
          &SVGDesigner::onZoomChanged);
  connect(m_fitButton, &QPushButton::clicked, this,
          &SVGDesigner::onFitToViewClicked);

  // Connect tool buttons
  connect(m_selectToolButton, &QPushButton::clicked, this,
          &SVGDesigner::onSelectToolClicked);
  connect(m_measureToolButton, &QPushButton::clicked, this,
          &SVGDesigner::onMeasureToolClicked);
  connect(m_panToolButton, &QPushButton::clicked, this,
          &SVGDesigner::onPanToolClicked);

  // Connect grid and snap controls
  connect(m_gridVisibleCheckBox, &QCheckBox::toggled, this,
          &SVGDesigner::onGridVisibilityChanged);
  connect(m_gridSpacingSpinBox,
          QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
          &SVGDesigner::onGridSpacingChanged);
  connect(m_snapModeComboBox,
          QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &SVGDesigner::onSnapModeChanged);
}
...
bool SVGDesigner::isSvgLoaded() const { return m_designerView->isSvgLoaded(); }
...
void SVGDesigner::onOpenSvgClicked() {
  QString filePath = QFileDialog::getOpenFileName(
      this, tr("Open SVG File"), QString(), tr("SVG Files (*.svg)"));
  if (!filePath.isEmpty()) {
...
void SVGDesigner::onConvertClicked() {
  ...
  QPointF designOffset =
      designBounds.topLeft();  // Top-left in screen coordinates
  ...
  if (designBounds.left() < 0) {
    warnings << tr("Design extends %1 mm beyond the left edge of the material")
                    .arg(-designBounds.left(), 0, 'f', 2);
    fitsInMaterial = false;
  }
  if (designBounds.top() < 0) {
    warnings << tr("Design extends %1 mm beyond the top edge of the material")
                    .arg(-designBounds.top(), 0, 'f', 2);
    fitsInMaterial = false;
  }
  if (designBounds.right() > materialWidth) {
    warnings << tr("Design extends %1 mm beyond the right edge of the material")
                    .arg(designBounds.right() - materialWidth, 0, 'f', 2);
    fitsInMaterial = false;
  }
  if (designBounds.bottom() > materialHeight) {
    warnings
        << tr("Design extends %1 mm beyond the bottom edge of the material")
               .arg(designBounds.bottom() - materialHeight, 0, 'f', 2);
    fitsInMaterial = false;
  }
  ...
  // Note: G-code generation is now handled by the unified G-code options panel
  // system Users should use the "Generate G-Code" button in the G-code options
  // panel
}
...
void SVGDesigner::updateMaterialFromSettings() {
  // This method would be connected to the GCodeOptionsPanel to update material
  // settings Implementation depends on how the GCodeOptionsPanel is structured
}
...
QPair<QRectF, QRectF> DesignerView::calculateLabelPositions(
    const QString &materialLabel, const QString &bedLabel,
  ...
  double materialPadding =
      getZoomAdjustedSize(qMin(materialMinDimension * 0.05, 5.0));
  double bedPadding = getZoomAdjustedSize(qMin(bedMinDimension * 0.04, 4.0));

  // Position material label at top-left of material
  QRectF materialLabelRect = materialRect.adjusted(
      materialPadding, materialPadding, -materialPadding, -materialPadding);
  ...
  QRectF bedLabelRect =
      bedRect.adjusted(bedPadding, bedPadding * 2, -bedPadding, -bedPadding);
  ...
  if (materialLabelRect.intersects(bedLabelRect)) {
    ...
    // If they're similar sizes, move material to bottom-right and bed to
    // top-left
    else {
      materialLabelRect.moveRight(materialRect.right() - materialPadding);
      materialLabelRect.moveBottom(materialRect.bottom() - materialPadding);
