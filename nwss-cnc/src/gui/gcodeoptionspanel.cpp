...
GCodeOptionsPanel::GCodeOptionsPanel(QWidget *parent)
  ...
  horizontalLayout->addWidget(
      contentWidget, 39);  // Give content widget 38 parts (95% of space)
  horizontalLayout->addStretch(1);  // Very small stretch on the right

  // Connect signals
  connect(generateButton, &QPushButton::clicked, this,
          [this]() { emit generateGCode(m_currentSvgFile); });
  connect(toolTypeComboBox, &QComboBox::currentTextChanged, this,
          &GCodeOptionsPanel::onToolTypeChanged);
  connect(metricUnitsCheckBox, &QCheckBox::toggled, this,
          &GCodeOptionsPanel::updateUnitDisplay);
  connect(materialThicknessSpinBox,
          QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
          &GCodeOptionsPanel::onMaterialChanged);
  connect(cutDepthSpinBox, QOverload<double>::of(&QDoubleSpinBox::valueChanged),
          this, &GCodeOptionsPanel::recalculatePassCount);

  // Connect material and bed size changes to emit optionsChanged signal
  connect(materialWidthSpinBox,
          QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
          [this]() { emit optionsChanged(); });
  connect(materialHeightSpinBox,
          QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
          [this]() { emit optionsChanged(); });
  connect(bedWidthSpinBox, QOverload<double>::of(&QDoubleSpinBox::valueChanged),
          this, [this]() { emit optionsChanged(); });
  connect(bedHeightSpinBox,
          QOverload<double>::of(&QDoubleSpinBox::valueChanged), this,
          [this]() { emit optionsChanged(); });
...
void GCodeOptionsPanel::setupToolTab() {
  ...
  toolTypeComboBox->addItems(
      {"End Mill", "Ball End Mill", "V-Bit", "Drill Bit"});
...
void GCodeOptionsPanel::setupCuttingTab() {
  ...
  // Connect signals
  connect(safetyHeightCheckBox, &QCheckBox::toggled, safetyHeightSpinBox,
          &QDoubleSpinBox::setEnabled);
  connect(cutoutModeComboBox,
          QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &GCodeOptionsPanel::onCutoutModeChanged);
}
...
void GCodeOptionsPanel::setupTransformTab() {
  ...
  QGroupBox *transformGroup =
      new QGroupBox("Path Transformation", transformTab);
...
void GCodeOptionsPanel::setupAdvancedTab() {
  ...
  QGroupBox *discretizationGroup =
      new QGroupBox("Discretization Options", advancedTab);
...
void GCodeOptionsPanel::onMaterialChanged() { recalculatePassCount(); }

void GCodeOptionsPanel::recalculatePassCount() {
  // Calculate how many passes are needed based on material thickness and cut
  // depth
...
bool GCodeOptionsPanel::getFlipY() const { return flipYCheckBox->isChecked(); }
...
void GCodeOptionsPanel::setupProfileButtons() {
  ...
  connect(saveProfileButton, &QPushButton::clicked, this,
          &GCodeOptionsPanel::saveProfileAs);
  ...
  connect(loadProfileButton, &QPushButton::clicked, this,
          &GCodeOptionsPanel::loadProfile);
...
void GCodeOptionsPanel::saveSettings() {
  ...
  settings.setValue("PreserveAspectRatio",
                    preserveAspectRatioCheckBox->isChecked());
...
void GCodeOptionsPanel::loadSettings() {
  ...
  materialThicknessSpinBox->setValue(
      settings.value("Thickness", 6.0).toDouble());
  settings.endGroup();

  // Load Tool settings
  settings.beginGroup("ToolSettings");
  toolTypeComboBox->setCurrentText(
      settings.value("ToolType", "End Mill").toString());
  toolDiameterSpinBox->setValue(
      settings.value("ToolDiameter", 3.175).toDouble());
  ...
  spindleSpeedSpinBox->setValue(
      settings.value("SpindleSpeed", 10000.0).toDouble());
  cutDepthSpinBox->setValue(settings.value("CutDepth", 1.0).toDouble());
  passCountSpinBox->setValue(settings.value("PassCount", 6).toInt());
  safetyHeightCheckBox->setChecked(
      settings.value("SafetyHeightEnabled", true).toBool());
  ...
  simplifySpinBox->setValue(
      settings.value("SimplifyTolerance", 0.05).toDouble());
  maxPointDistanceSpinBox->setValue(
      settings.value("MaxPointDistance", 1.0).toDouble());
  settings.endGroup();

  // Load Path Transformation options
  settings.beginGroup("PathTransformationSettings");
  preserveAspectRatioCheckBox->setChecked(
      settings.value("PreserveAspectRatio", true).toBool());
  ...
  settings.beginGroup("GCodeGenerationSettings");
  optimizePathsCheckBox->setChecked(
      settings.value("OptimizePaths", true).toBool());
  linearizePathsCheckBox->setChecked(
      settings.value("LinearizePaths", true).toBool());
  settings.endGroup();
...
void GCodeOptionsPanel::saveProfileAs() {
  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Save Settings Profile"), "",
      tr("NWSS-CNC Profile (*.ncp);;All Files (*)"));

  if (fileName.isEmpty()) return;

  // Make sure the file has the correct extension
  if (!fileName.endsWith(".ncp", Qt::CaseInsensitive)) fileName += ".ncp";
  ...
  profile.setValue("PreserveAspectRatio",
                   preserveAspectRatioCheckBox->isChecked());
...
void GCodeOptionsPanel::loadProfile() {
  ...
  // Load Machine settings
  profile.beginGroup("MachineSettings");
  bedWidthSpinBox->setValue(
      profile.value("BedWidth", bedWidthSpinBox->value()).toDouble());
  bedHeightSpinBox->setValue(
      profile.value("BedHeight", bedHeightSpinBox->value()).toDouble());
  metricUnitsCheckBox->setChecked(
      profile.value("MetricUnits", metricUnitsCheckBox->isChecked()).toBool());
  profile.endGroup();

  // Load Material settings
  profile.beginGroup("MaterialSettings");
  materialWidthSpinBox->setValue(
      profile.value("Width", materialWidthSpinBox->value()).toDouble());
  materialHeightSpinBox->setValue(
      profile.value("Height", materialHeightSpinBox->value()).toDouble());
  materialThicknessSpinBox->setValue(
      profile.value("Thickness", materialThicknessSpinBox->value()).toDouble());
  profile.endGroup();

  // Load Tool settings
  profile.beginGroup("ToolSettings");
  toolTypeComboBox->setCurrentText(
      profile.value("ToolType", toolTypeComboBox->currentText()).toString());
  toolDiameterSpinBox->setValue(
      profile.value("ToolDiameter", toolDiameterSpinBox->value()).toDouble());
  toolFluteCountSpinBox->setValue(
      profile.value("FluteCount", toolFluteCountSpinBox->value()).toInt());
  profile.endGroup();

  // Load Cutting settings
  profile.beginGroup("CuttingSettings");
  feedRateSpinBox->setValue(
      profile.value("FeedRate", feedRateSpinBox->value()).toDouble());
  plungeRateSpinBox->setValue(
      profile.value("PlungeRate", plungeRateSpinBox->value()).toDouble());
  spindleSpeedSpinBox->setValue(
      profile.value("SpindleSpeed", spindleSpeedSpinBox->value()).toDouble());
  cutDepthSpinBox->setValue(
      profile.value("CutDepth", cutDepthSpinBox->value()).toDouble());
  passCountSpinBox->setValue(
      profile.value("PassCount", passCountSpinBox->value()).toInt());
  safetyHeightCheckBox->setChecked(
      profile.value("SafetyHeightEnabled", safetyHeightCheckBox->isChecked())
          .toBool());
  safetyHeightSpinBox->setValue(
      profile.value("SafetyHeight", safetyHeightSpinBox->value()).toDouble());
  profile.endGroup();

  // Load Discretization options
  profile.beginGroup("DiscretizationSettings");
  bezierSamplesSpinBox->setValue(
      profile.value("BezierSamples", bezierSamplesSpinBox->value()).toInt());
  adaptiveSpinBox->setValue(
      profile.value("AdaptiveSampling", adaptiveSpinBox->value()).toDouble());
  simplifySpinBox->setValue(
      profile.value("SimplifyTolerance", simplifySpinBox->value()).toDouble());
  maxPointDistanceSpinBox->setValue(
      profile.value("MaxPointDistance", maxPointDistanceSpinBox->value())
          .toDouble());
  profile.endGroup();
  ...
  preserveAspectRatioCheckBox->setChecked(
      profile
          .value("PreserveAspectRatio",
                 preserveAspectRatioCheckBox->isChecked())
          .toBool());
  centerXCheckBox->setChecked(
      profile.value("CenterX", centerXCheckBox->isChecked()).toBool());
  centerYCheckBox->setChecked(
      profile.value("CenterY", centerYCheckBox->isChecked()).toBool());
  flipYCheckBox->setChecked(
      profile.value("FlipY", flipYCheckBox->isChecked()).toBool());
  profile.endGroup();

  // Load G-Code Generation options
  profile.beginGroup("GCodeGenerationSettings");
  optimizePathsCheckBox->setChecked(
      profile.value("OptimizePaths", optimizePathsCheckBox->isChecked())
          .toBool());
  linearizePathsCheckBox->setChecked(
      profile.value("LinearizePaths", linearizePathsCheckBox->isChecked())
          .toBool());
  profile.endGroup();
