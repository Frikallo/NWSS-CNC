...
void GCodeViewer3D::initializeGL() {
  ...
  gridVbo.create();
  gridVbo.bind();
  gridVbo.allocate(nullptr,
                   4096 * sizeof(float));  // Allocate space for grid lines
  gridVao.release();
...
void GCodeViewer3D::initViewCube() {
  ...
  bottomFace.vertices = {vertices[20], vertices[21], vertices[22],
                         vertices[23]};
  ...
  frontLeftFace.vertices = {vertices[0], vertices[3], vertices[11],
                            vertices[8]};
  ...
  frontRightFace.vertices = {vertices[1], vertices[12], vertices[15],
                             vertices[2]};
  ...
  frontTopFace.vertices = {vertices[3], vertices[2], vertices[17],
                           vertices[16]};
  ...
  frontBottomFace.vertices = {vertices[0], vertices[1], vertices[21],
                              vertices[20]};
  ...
  backRightFace.vertices = {vertices[5], vertices[6], vertices[14],
                            vertices[13]};
  ...
  backBottomFace.vertices = {vertices[4], vertices[5], vertices[22],
                             vertices[23]};
  ...
  leftTopFace.vertices = {vertices[11], vertices[10], vertices[19],
                          vertices[16]};
  ...
  leftBottomFace.vertices = {vertices[8], vertices[9], vertices[23],
                             vertices[20]};
  ...
  rightTopFace.vertices = {vertices[15], vertices[14], vertices[18],
                           vertices[17]};
  ...
  rightBottomFace.vertices = {vertices[12], vertices[13], vertices[22],
                              vertices[21]};
...
void GCodeViewer3D::drawViewCube(QPainter *painter) {
  ...
  painter->translate(m_cubePosition.x() + m_cubeSize / 2,
                     m_cubePosition.y() + m_cubeSize / 2);
  ...
  painter->drawEllipse(QPointF(0, 0), m_cubeSize / 2 * 1.1,
                       m_cubeSize / 2 * 1.1);
  ...
  for (int idx : faceOrder) {
    ...
    for (const QVector3D &v : face.vertices) {
      ...
      polygon << QPointF(rotated.x() * m_cubeSize / 2,
                         rotated.y() * m_cubeSize / 2);
    }

    // Determine if face is oriented toward camera (for label visibility)
    QVector3D normalRotated = cubeMatrix.map(face.normal);
    bool isFaceVisible =
        normalRotated.z() < -0.05;  // Only visible if somewhat facing camera
    ...
    if (isFaceVisible && !face.label.isEmpty()) {
      ...
      painter->drawText(QPointF(labelPos.x() * m_cubeSize / 2 -
  ...
  // painter->drawLine(QPointF(0, 0), QPointF(xAxis.x() * m_cubeSize/2,
  // xAxis.y() * m_cubeSize/2));

  // painter->setPen(QPen(Qt::green, 2));
  // QVector3D yAxis = cubeMatrix.map(QVector3D(0, 1.5, 0));
  // painter->drawLine(QPointF(0, 0), QPointF(yAxis.x() * m_cubeSize/2,
  // yAxis.y() * m_cubeSize/2));

  // painter->setPen(QPen(Qt::blue, 2));
  // QVector3D zAxis = cubeMatrix.map(QVector3D(0, 0, 1.5));
  // painter->drawLine(QPointF(0, 0), QPointF(zAxis.x() * m_cubeSize/2,
  // zAxis.y() * m_cubeSize/2));
...
bool GCodeViewer3D::isPointInViewCube(const QPoint &point) {
  ...
  QPointF center(m_cubePosition.x() + m_cubeSize / 2,
                 m_cubePosition.y() + m_cubeSize / 2);
...
int GCodeViewer3D::getCubeFaceAtPoint(const QPoint &point) {
  ...
  QPointF localPoint(point.x() - (m_cubePosition.x() + m_cubeSize / 2),
  ...
  for (int i = 0; i < m_cubeViewFaces.size(); i++) {
    ...
    if (polygon.containsPoint(localPoint, Qt::OddEvenFill)) {
      ...
      // Update closest face if this one is closer to the camera (smaller
      // z-value)
...
QVector3D GCodeViewer3D::mapToSphere(const QPointF &point) {
  ...
  QPointF cubeCenter(m_cubePosition.x() + m_cubeSize / 2,
                     m_cubePosition.y() + m_cubeSize / 2);
...
void GCodeViewer3D::rotateCubeByConstrainedAxes(int deltaX, int deltaY) {
  ...
  float yawAngle =
      -deltaX * sensitivity;  // Horizontal movement -> Z-axis rotation (yaw) -
                              // negative for proper CAD feel
  float pitchAngle =
      -deltaY * sensitivity;  // Vertical movement -> X-axis rotation (pitch) -
                              // negative for proper CAD feel

  // Create separate rotations for yaw and pitch in world space
  // For CAD-style rotation: drag left = rotate counter-clockwise around Z, drag
  // up = rotate around X
  QQuaternion yawRotation = QQuaternion::fromAxisAndAngle(
      QVector3D(0, 0, 1), yawAngle);  // Yaw around world Z-axis
  QQuaternion pitchRotation = QQuaternion::fromAxisAndAngle(
      QVector3D(1, 0, 0), pitchAngle);  // Pitch around world X-axis
...
void GCodeViewer3D::animateToView(const QVector3D &targetPosition,
                                  const QVector3D &targetTarget) {
...
void GCodeViewer3D::updateAnimation() {
  ...
  cameraPosition = m_startCameraPosition +
                   smoothT * (m_targetCameraPosition - m_startCameraPosition);
  cameraTarget = m_rotationCenter;  // Always keep target at rotation center

  // Interpolate orientation (slerp) - this is the key to smooth synchronized
  // rotation
  m_viewOrientation =
      QQuaternion::slerp(m_startOrientation, m_targetOrientation, smoothT);
...
void GCodeViewer3D::setIsometricView() {
  ...
  cameraPosition =
      m_rotationCenter +
      isoDirection * distance;  // Camera looks FROM this position TO the target

  // For a proper right-side-up view, we need to set up the orientation
  // correctly The camera should look from the isometric position toward the
  // target
  QVector3D lookDirection = (cameraTarget - cameraPosition).normalized();

  // Calculate the corresponding orientation quaternion
  QVector3D defaultDirection(0.0f, -1.0f,
                             0.0f);  // Default camera looks along negative Y
  ...
  if (axis.length() < 0.001f) {
    ...
    axis.normalize();
    float angle = acos(qBound(-1.0,
                              (double)QVector3D::dotProduct(defaultDirection,
                                                            lookDirection),
                              1.0)) *
                  180.0f / M_PI;
...
void GCodeViewer3D::setupGridShaders() {
  ...
  if (!gridProgram.addShaderFromSourceCode(QOpenGLShader::Vertex,
                                           gridVertexShaderSource)) {
    qDebug() << "Failed to compile grid vertex shader";
  }

  if (!gridProgram.addShaderFromSourceCode(QOpenGLShader::Fragment,
...
void GCodeViewer3D::setupPathShaders() {
  ...
  if (!pathProgram.addShaderFromSourceCode(QOpenGLShader::Vertex,
                                           pathVertexShaderSource)) {
    qDebug() << "Failed to compile path vertex shader";
  }

  if (!pathProgram.addShaderFromSourceCode(QOpenGLShader::Fragment,
                                           pathFragmentShaderSource)) {
...
void GCodeViewer3D::drawGrid() {
  ...
  for (int i = 0; i <= lineCount; i++) {
    ...
    if (std::abs(pos) < 0.001f)
      continue;  // Skip center lines (already drawn as axes)
  ...
  gridProgram.setAttributeBuffer(1, GL_FLOAT, 3 * sizeof(float), 3,
                                 6 * sizeof(float));
...
void GCodeViewer3D::rebuildGeometryForCurrentLOD() {
  ...
  if (m_useSimplifiedGeometry) {
    ...
    if (pathSize > 500000) {
      ...
      if (scale < 0.1f)
        m_simplificationFactor = 200;
      else if (scale < 0.3f)
        m_simplificationFactor = 100;
      else if (scale < 0.5f)
        m_simplificationFactor = 50;
      else if (scale < 0.8f)
        m_simplificationFactor = 20;
      else
        m_simplificationFactor = 10;
  ...
  if (toolPath.size() > 10000 && m_simplificationFactor > 1) {
    ...
    float epsilon =
        0.5f *
        m_simplificationFactor;  // Tolerance increases with simplification

    // First pass: mark key points (important direction changes and type
    // changes)
    ...
    for (size_t i = 1; i < toolPath.size() - 1; i++) {
      ...
      QVector3D v1 =
          (toolPath[i].position - toolPath[i - 1].position).normalized();
      QVector3D v2 =
          (toolPath[i + 1].position - toolPath[i].position).normalized();
      ...
      if (dot <
          0.98f -
              0.02f *
                  m_simplificationFactor) {  // Adaptive threshold based on LOD
        isKeyPoint[i] = true;
        continue;
      }
  ...
  // Reserve reasonable space (each point will be part of at most one line
  // segment)
...
void GCodeViewer3D::makePathVertices() {
  ...
  pathVertices.reserve(m_simplifiedToolPath.size() *
                       6);  // 3 pos + 3 color per vertex
  ...
  if (isValid() && !pathVertices.empty()) {
    ...
    pathProgram.setAttributeBuffer(1, GL_FLOAT, 3 * sizeof(float), 3,
                                   6 * sizeof(float));
    ...
    int totalIndexSize =
        (m_rapidIndexCount + m_cuttingIndexCount) * sizeof(int);
    ...
    if (m_rapidIndexCount > 0) {
      pathIndexBuffer.write(m_rapidIndexOffset, m_indicesRapid.data(),
                            m_rapidIndexCount * sizeof(int));
    }

    // Upload cutting move indices
    if (m_cuttingIndexCount > 0) {
      pathIndexBuffer.write(m_cuttingIndexOffset, m_indicesCutting.data(),
                            m_cuttingIndexCount * sizeof(int));
    }
...
void GCodeViewer3D::mouseMoveEvent(QMouseEvent *event) {
  ...
  // Performance optimization for large models: reduce operations during
  // dragging
  ...
  if (m_isDraggingCube) {
    ...
    if (event->modifiers() & Qt::ShiftModifier) {
      ...
      QVector3D movement =
          -rightVector * dx * panSpeed + upVector * dy * panSpeed;
...
void GCodeViewer3D::animateToViewDirection(const QVector3D &direction) {
  ...
  if (rotAxis.length() < 0.001f) {
    ...
    rotAxis.normalize();
    float angle =
        acos(QVector3D::dotProduct(initialDir, lookDir)) * 180.0f / M_PI;
...
void GCodeViewer3D::processGCode(const QString &gcode) {
  ...
  try {
    parseGCode(gcode);
    pathNeedsUpdate = true;
    updateTimer->start(100);  // Wait a bit before updating to avoid multiple
...
void GCodeViewer3D::autoScaleToFit() {
  ...
  qDebug() << "Auto-scaled to fit. Model size:" << diagonal
           << "New scale:" << scale;
...
void GCodeViewer3D::parseGCode(const QString &gcode) {
  ...
  try {
    ...
    toolPath.reserve(lineCount *
                     0.5);  // Assuming about 50% of lines contain movement
    ...
    // More efficient line splitting - avoids creating a QStringList with all
    // lines
    int pos = 0;
    int nextLinePos;

    // Pre-compile regular expressions for better performance
    static QRegularExpression g0Regex(
        "G0[\\s]|G00[\\s]", QRegularExpression::CaseInsensitiveOption);
    static QRegularExpression g1Regex(
        "G1[\\s]|G01[\\s]", QRegularExpression::CaseInsensitiveOption);
    static QRegularExpression g20Regex(
        "G20[\\s]", QRegularExpression::CaseInsensitiveOption);
    static QRegularExpression g21Regex(
        "G21[\\s]", QRegularExpression::CaseInsensitiveOption);
    static QRegularExpression g90Regex(
        "G90[\\s]", QRegularExpression::CaseInsensitiveOption);
    static QRegularExpression g91Regex(
        "G91[\\s]", QRegularExpression::CaseInsensitiveOption);
    ...
    while (pos < gcode.length()) {
      ...
      QRegularExpressionMatchIterator coordMatches =
          coordPattern.globalMatch(line);
      ...
      if (hasMovement) {
        ...
        if (hasZMovement && !hasXYMovement && hasLastCoordinates) {
          ...
          // If this is a Z plunge (G01) that's about to be followed by a
          // cutting move, we need to add a duplicate point with isRapid=false
