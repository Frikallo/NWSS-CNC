...
int ToolManager::getSelectedToolId() const { return m_selectedToolId; }
...
void ToolManager::setupToolTable() {
  ...
  QStringList headers;
  headers << "ID" << "Name" << "Type" << "Diameter (mm)" << "Material"
          << "Active";
  ...
  connect(m_toolTable, &QTableWidget::itemSelectionChanged, this,
          &ToolManager::onToolSelectionChanged);
  connect(m_toolTable, &QTableWidget::cellDoubleClicked, this,
          &ToolManager::onToolDoubleClicked);
...
void ToolManager::setupToolEditor() {
  ...
  m_typeCombo->addItem("Engraving Bit",
                       static_cast<int>(ToolType::ENGRAVING_BIT));
...
void ToolManager::setupButtons() {
  ...
  connect(m_deleteButton, &QPushButton::clicked, this,
          &ToolManager::onDeleteTool);
  connect(m_duplicateButton, &QPushButton::clicked, this,
          &ToolManager::onDuplicateTool);
  connect(m_importButton, &QPushButton::clicked, this,
          &ToolManager::onImportTools);
  connect(m_exportButton, &QPushButton::clicked, this,
          &ToolManager::onExportTools);
...
void ToolManager::loadToolToEditor(const Tool &tool) {
  ...
  m_typeCombo->setCurrentIndex(
      m_typeCombo->findData(static_cast<int>(tool.type)));
  ...
  m_materialCombo->setCurrentIndex(
      m_materialCombo->findData(static_cast<int>(tool.material)));
  m_coatingCombo->setCurrentIndex(
      m_coatingCombo->findData(static_cast<int>(tool.coating)));
...
Tool ToolManager::getToolFromEditor() const {
  ...
  tool.material =
      static_cast<ToolMaterial>(m_materialCombo->currentData().toInt());
  tool.coating =
      static_cast<ToolCoating>(m_coatingCombo->currentData().toInt());
...
void ToolManager::updateToolInTable(int row, const Tool &tool) {
  m_toolTable->setItem(row, COLUMN_ID,
                       new QTableWidgetItem(QString::number(tool.id)));
  m_toolTable->setItem(row, COLUMN_NAME,
                       new QTableWidgetItem(QString::fromStdString(tool.name)));
  m_toolTable->setItem(
      row, COLUMN_TYPE,
      new QTableWidgetItem(QString::fromStdString(tool.getTypeString())));
  m_toolTable->setItem(
      row, COLUMN_DIAMETER,
      new QTableWidgetItem(QString::number(tool.diameter, 'f', 3)));
  m_toolTable->setItem(
      row, COLUMN_MATERIAL,
      new QTableWidgetItem(QString::fromStdString(tool.getMaterialString())));
  m_toolTable->setItem(row, COLUMN_ACTIVE,
                       new QTableWidgetItem(tool.isActive ? "Yes" : "No"));
...
void ToolManager::onImportTools() {
  QString fileName = QFileDialog::getOpenFileName(
      this, "Import Tools", "", "Tool Files (*.tools);;All Files (*)");
  ...
  if (!fileName.isEmpty()) {
    if (m_toolRegistry.loadFromFile(fileName.toStdString())) {
      ...
      QMessageBox::warning(this, "Import Failed",
                           "Failed to import tools from file.");
    }
  }
}
...
void ToolManager::onExportTools() {
  QString fileName =
      QFileDialog::getSaveFileName(this, "Export Tools", "tools.tools",
                                   "Tool Files (*.tools);;All Files (*)");
  ...
  if (!fileName.isEmpty()) {
    if (m_toolRegistry.saveToFile(fileName.toStdString())) {
      ...
      QMessageBox::warning(this, "Export Failed",
                           "Failed to export tools to file.");
    }
  }
}
...
ToolSelector::ToolSelector(ToolRegistry &toolRegistry, QWidget *parent)
    : QWidget(parent), m_toolRegistry(toolRegistry) {
  setupUI();
  populateToolCombo();
}
...
void ToolSelector::refreshTools() { populateToolCombo(); }
...
void ToolSelector::onToolChanged() { emit toolSelected(getSelectedToolId()); }
...
void ToolSelector::onManageTools() {
  ...
  connect(&manager, &ToolManager::toolRegistryChanged, this,
