...
bool Transform::getBounds(const std::vector<Path>& paths, 
...
bool Transform::fitToMaterial(std::vector<Path>& paths, 
    ...
    if (exceedsWidth || exceedsHeight) {
        ...
        if (preserveAspectRatio) {
            ...
            for (auto& path : paths) {
                std::vector<Point2D>& points = const_cast<std::vector<Point2D>&>(path.getPoints());
...
std::string Transform::formatTransformInfo(const TransformInfo& info, const CNConfig& config) {
    std::stringstream ss;
    ...
    // Original dimensions
    ss << "Original dimensions: " 
       << std::fixed << std::setprecision(3) 
       << info.origWidth << " x " << info.origHeight << " " << units << std::endl;
    
    ss << "Original position: (" 
       << info.origMinX << ", " << info.origMinY << ") " << units << std::endl;
    ...
    ss << "New position: (" 
       << info.newMinX << ", " << info.newMinY << ") " << units << std::endl;
    ...
    // Material size
    ss << "Material size: " 
       << config.getMaterialWidth() << " x " << config.getMaterialHeight() 
       << " " << units << std::endl;
    
    // Bed size
    ss << "Bed size: " 
       << config.getBedWidth() << " x " << config.getBedHeight() 
       << " " << units << std::endl;
    
    // Warning if needed
    if (info.wasCropped) {
        ss << std::endl << "WARNING: The design exceeds the bed dimensions!" << std::endl;
