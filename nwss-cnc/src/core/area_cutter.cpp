...
std::vector<Path> AreaCutter::generateContourPaths(const Polygon& polygon,
    ...
    return m_camProcessor->generateContourToolpath(polygon, toolDiameter, stepover);
}
...
std::vector<Path> AreaCutter::generateSpiralPaths(const Polygon& polygon,
    ...
    return m_camProcessor->generateSpiralToolpath(polygon, toolDiameter, stepover, inward);
}
...
std::vector<Path> AreaCutter::generateRasterPaths(const Polygon& polygon,
    ...
    return m_camProcessor->generateRasterToolpath(polygon, toolDiameter, stepover, angle);
}
...
bool AreaCutter::validateCutParameters(const Polygon& polygon,
    ...
    auto validationResult = m_camProcessor->validateToolpathFeasibility(polygon, toolDiameter, mode);
