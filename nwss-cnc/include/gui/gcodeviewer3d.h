...
class GCodeViewer3D : public QOpenGLWidget, protected QOpenGLFunctions {
 ...
 private:
  ...
  void animateToView(const QVector3D &targetPosition,
                     const QVector3D &targetTarget);
  void updateAnimation();

  // View cube rotation methods
  QVector3D mapToSphere(
      const QPointF &point);  // Legacy arcball method (kept for compatibility)
  void rotateCubeByMouse(const QPoint &from,
                         const QPoint &to);  // Constrained-axis rotation
  void rotateCubeByConstrainedAxes(
      int deltaX,
      int deltaY);  // Helper for constrained rotation
  ...
  GCodePoint interpolatePoints(const GCodePoint &a, const GCodePoint &b,
                               float t);
