...
struct GCodeOptions {
  ...
  double stepover;  // Stepover distance for area cutting (as fraction of tool
                    // diameter)
...
class GCodeGenerator {
 public:
   ...
   */
  bool generateGCode(const std::vector<Path> &paths,
   ...
   */
  bool validatePaths(const std::vector<Path> &paths,
 ...
 private:
   ...
   */
  void linearizePath(std::ostream &out, const std::vector<Point2D> &points,
   ...
   */
  bool isCollinear(const Point2D &p1, const Point2D &p2,
   ...
   */
  std::vector<Path> generateAreaCuttingPaths(
