...
class Transform {
 public:
   ...
   */
  static bool getBounds(const std::vector<Path> &paths, double &minX,
   ...
   * @param flipY Whether to flip the Y coordinates (for correct CNC
   * orientation)
   * @param info Output parameter for transformation information
   * @return true if transformation was successful
   */
  static bool fitToMaterial(std::vector<Path> &paths, const CNConfig &config,
                            bool preserveAspectRatio = true,
                            bool centerX = true, bool centerY = true,
                            bool flipY = true, TransformInfo *info = nullptr);
