...
struct CutoutParams {
  ...
  double stepover =
      0.5;  // Stepover distance for area cutting (as fraction of tool diameter)
  ...
  CutoutParams(CutoutMode m, double so = 0.5, double ol = 0.1, bool si = true,
               double mso = 2.0)
...
class Path {
 public:
  ...
  void addPoint(const Point2D &point) { m_points.push_back(point); }

  // Get all points
  const std::vector<Point2D> &getPoints() const { return m_points; }

  // Get a specific point
  const Point2D &getPoint(size_t index) const { return m_points.at(index); }
